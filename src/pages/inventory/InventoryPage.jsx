...
export default function InventoryPage() {
  ...
  const ITEM_COLS = [
    ...
    { key: 'quantity', label: 'Stock',    render: (v, row) => (
      <span style={{ color: parseFloat(v) <= parseFloat(row.reorder_level) ? 'var(--color-accent-orange)' : 'var(--color-accent-green)', fontWeight: 600 }}>
        {v} {row.unit}
      </span>
    )},
  ...
  return (
    <div className="inventory-page page-enter">
            ...
            <DataTable
              ...
              actions={canManage ? (row) => (
                <>
                  ...
                  <button className="btn-icon" style={{ color: 'var(--color-accent-red)' }} onClick={() => setDeleteTarget(row)}><MdDelete /></button>
                </>
              ) : undefined}
            />
          )}
      ...
      <Modal
        ...
        title={editItem ? 'Edit Item' : 'New Inventory Item'}
        size="lg"
        footer={
          <>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div className="form-row">
            <FormField label="Item Name" error={touched.item_name && errors.item_name} required htmlFor="inv-name">
              <input
                ...
                placeholder="e.g. Acrylic Paint — White"
              />
            </FormField>
            <FormField label="Category" error={touched.category && errors.category} required htmlFor="inv-cat">
              <input
                id="inv-cat"
                type="text"
                value={form.category}
                onChange={(e) => setField('category', e.target.value)}
                placeholder="e.g. Paints, Primers, Tools"
              />
            </FormField>
          </div>
          ...
          <div className="form-row">
            <FormField label="Supplier (optional)" htmlFor="inv-sup">
            ...
            {editItem && (
              <FormField label="SKU" htmlFor="inv-sku">
                <input id="inv-sku" type="text" value={form.sku || ''} disabled />
              </FormField>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        loading={deleting}
        title="Delete Item"
        message={`Delete "${deleteTarget?.item_name}"? This cannot be undone.`}
        variant="danger"
      />

      <ConfirmDialog
        open={Boolean(deleteMovTarget)}
        onClose={() => setDeleteMovTarget(null)}
