...
router.post('/', authorize(ROLES), auditLog('CREATE', 'journal_entries'),
  [
...
router.put('/:id', authorize(ROLES), auditLog('UPDATE', 'journal_entries'), param('id').isInt(), validate, ctrl.updateEntry);
router.post('/:id/post', authorize(ROLES), auditLog('UPDATE', 'journal_entries'), param('id').isInt(), validate, ctrl.postEntry);
